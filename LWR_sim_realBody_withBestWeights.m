...
index=index-1;
acc=zeros(1,7);
d2p_ref =[   -0.0494    0.0077         0]';
        Kp=[10,0,0;0,20,0;0,0,20];
        Kd=Kp;
        Point_intersected=Point_intersected';
        tic
while (toc<240)%(frequency * (t0) < 2*pi) % it ends when a circle is completed
     disp('time instant:')
...
%          end
   ...
   %TauExternalForce=[1 0 0 0 0  0 0 ];
    ...
    %% Residual Calculation and Force and point reconstruction
     % in order to have enough samples to calculate the residuals
            ...
            Rotation = T(1:3,1:3);
             tran = T(1:3,4);
               ...
               % p_dc=pinv(-Sf_i)*m_i
                ...
                figure(f3),plot3( Point_intersected(1), Point_intersected(2),Point_intersected(3), 'o', 'MarkerSize', 4, 'MarkerFaceColor', 'g', 'LineWidth', 2);
