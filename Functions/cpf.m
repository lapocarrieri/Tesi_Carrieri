...
function [chi, W_prime] = cpf(num_part, chi_prev, q, gamma, estimated_cp,link,is_initialized)
a=1
    ...
    if(~is_initialized)
        ...
        chi = chi_prev;
        for i=1:num_part
            %theta_v =theta_hat +pi*rand(); 
            %theta_h=theta_hat;%uniform distr on one part of the cylinder
            theta_v = normrnd(theta_hat, 0.5); %gaussian distribution around the estimated contact point
            sigma=(height-0)/3;
            
            z = normrnd(estimated_cp(3), abs(sigma));
                while (z > 0 || z < height)
                 z = normrnd(estimated_cp(3),abs(sigma));
                end
