...
Y1=[data(1).point(1)' * ones(length(X1),1),data(1).point(2)' * ones(length(X1),1),data(1).point(3)' * ones(length(X1),1)]

X2=data(2).Residual_calculated;
Y2=[ data(2).point(1)' * ones(length(X2),1),data(2).point(2)' * ones(length(X2),1),data(2).point(3)' * ones(length(X2),1)]

X3=data(3).Residual_calculated; % Nx7 matrix of input vectors
Y3=[ data(3).point(1)' * ones(length(X3),1),data(3).point(2)' * ones(length(X3),1),data(3).point(3)' * ones(length(X3),1)]

X4=data(4).Residual_calculated; % Nx7 matrix of input vectors
Y4=[data(4).point(1)' * ones(length(X4),1),data(4).point(2)' * ones(length(X4),1),data(4).point(3)' * ones(length(X4),1)]

X5=data(5).Residual_calculated; % Nx7 matrix of input vectors
Y5=[ data(5).point(1)' * ones(length(X5),1),data(5).point(2)' * ones(length(X5),1),data(5).point(3)' * ones(length(X5),1)]
